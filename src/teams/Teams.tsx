...
const TeamCard = (props: TeamCardProps) => {
    return (
        <li className="bg-dark-surface-variant rounded-lg text-white flex flex-col pt-2 m-4 ring-1 ring-inset ring-white/[.3] transform transition-all hover:-translate-y-2 duration-300 hover:ring-dark-primary">
            {props.children}
        </li>
    );
};
        ...
        <div className="bg-dark-surface text-center">
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 md:flex-row mx-8 bg-dark-surface pb-4">
                { teams[year].list.map((team, index) => {
                        return (
                            <TeamCard key={index}>  
                                {team.photo && <img className="mx-auto relative object-cover w-64 h-64 rounded-lg" src={team.photo} alt={`${team.name} photo`}/>}
