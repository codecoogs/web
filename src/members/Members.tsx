...
const LoadingScreen = ({ submitProps, stripeUrl }: { submitProps: SubmitProps, stripeUrl: string }) => {
  return (
    <div className="fixed flex justify-center items-center flex-col top-0 left-0 h-full w-full bg-opacity-60 bg-black z-20">
      ...
      {stripeUrl !== "" && <a href={stripeUrl} onClick={() => submitProps.setSubmitProcess(false)} target="_blank" className="text-dark-primary">Click here if you didn't get redirected</a>}
    </div>
  );
};
...
const Members = () => {
  ...
  return (
    <div className="text-white p-8">
      {submitProcess == true && <LoadingScreen submitProps={submitProps} stripeUrl={stripeUrl} />}
      ...
      <div className="bg-dark-surface flex flex-col mt-8 md:flex-row md:gap-8 md:p-10">
        <div className="flex-1 basis-1/2 md:mt-0"> 
          <MembershipForm submitProps={submitProps} stripeProps={stripeUrlProp} />
        </div>
